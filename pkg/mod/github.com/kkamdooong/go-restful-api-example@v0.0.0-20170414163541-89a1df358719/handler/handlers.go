...
func GetFinancings(w http.ResponseWriter, _ *http.Request) {
	financings := db.FindAll()

	bytes, err := json.Marshal(financings)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
...
func SaveFinancings(w http.ResponseWriter, r *http.Request) {
	body, err := ioutil.ReadAll(r.Body)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}

	com := new(model.Financing)
	err = json.Unmarshal(body, com)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
